...
with open('dataset/{}/train.json'.format(DATASET), 'r') as train, \
    ...
    for sentence in sentences:
        ...
        for entity in entities:
            values = entity[VALUE_KEY].split(' ')
            txt_value = txt[entity[START_KEY]-1:entity[END_KEY]].strip()
            for i, v in enumerate(values):
                if i == 0:
                    txt_value = txt_value.replace(v, "[{}@B-{}]".format(v.replace(' ', '#'), entity['entity']))
                else:
                    txt_value = txt_value.replace(v, "[{}@I-{}]".format(v.replace(' ', '#'), entity['entity']))
                print('txt_value', txt_value)
            txt_after = txt_after.replace(entity[VALUE_KEY], txt_value)
            print('txt_after', txt_after)
        print(txt)
        # print(txt_after)
        txt_after = txt_after.replace('?', ' ?')
        print(txt_after)
...
with open('dataset/{}/test.json'.format(DATASET), 'r') as test, \
    ...
    for sentence in sentences:
        ...
        for entity in entities:
            ...
            for i, v in enumerate(values):
                if i == 0:
                    txt_value = txt_value.replace(v, "[{}@B-{}]".format(v, entity['entity']))
                else:
                    txt_value = txt_value.replace(v, "[{}@I-{}]".format(v, entity['entity']))
        ...
        for line in lines:
            ...
            if cleaned_line == line:
                ...
                cleaned_line = cleaned_line.split('@')
                test_parsed.write('{}\t{}\n'.format(cleaned_line[0], cleaned_line[1]))
        test_parsed.write('\n')